...
export function registerLagerTools(server: McpServer, client: LagerClient): void {
  ...
  read('lager_saet', 'Sættets indhold', 'Dele med «med sættet»/«uden for», pakkelisten og hvad der mangler. Ingen priser.', { tag: z.string().min(1) }, (i) => client.get(`/saet/${encodeURIComponent(String(i.tag))}`));
  read('lager_ansvarlig_pladser', 'Lageransvarlig: pladser', 'Zoner (opbevaring og særlige: MOD, UDL, LAD, KLG, SRV, KAR, ASB-K, ASB-O), reoler og hylder med antal enheder.', { lagerId }, (i) => client.get('/ansvarlig/pladser', { lager: num(i.lagerId) }));
  ...
  write('lager_ansvarlig_saet_paa_plads', 'Lageransvarlig: sæt på plads', 'Giver enhederne pladsen som hjemmeplads (lagerarbejde uden beregning). Afvises, hvis reglerne siger nej (asbest, karantæne).', { lagerId, koder, plads: z.string().min(1).describe('Pladsens kode, fx A-12-3 eller ASB-O.') }, (i) => client.post('/ansvarlig/intern', { lagerId: i.lagerId, slags: 'flyt', linjer: i.koder, plads: i.plads, begrundelse: 'Sat på plads via MCP' }));
